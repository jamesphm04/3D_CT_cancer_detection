...
class LunaTrainingApp:
    ...
    def main(self):
        ...
        for epoch_ndx in range(1, self.cli_args.epochs + 1):
            log.info(f'Epoch {epoch_ndx} of {self.cli_args.epochs}, {len(train_dl)}/{len(val_dl)} batches of size {self.cli_args.batch_size}*{(torch.cuda.device_count() if self.use_cuda else 1)}')
            
            train_metrics_t = self.doTraining(epoch_ndx, train_dl)
            self.logMetrics(epoch_ndx, 'train', train_metrics_t)
    ...
    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g):
        ...
        input_g = input_t.to(self.device, non_blocking=True) #non_blocking is for performing other computations while the data transfer to the GPU is ongoing
        label_g = label_t.to(self.device, non_blocking=True)
        
        logits_g, propability_g = self.model(input_g)
        
        loss_func = nn.CrossEntropyLoss(reduction='none')
        loss_g = loss_func(logits_g, label_g[:, 1]) #index of the one-hot encoder 
        
        start_ndx = batch_ndx * batch_size
        end_ndx = start_ndx + label_t.size(0)
        
        metrics_g[METRICS_LABEL_NDX, start_ndx:end_ndx] = label_g[:, 1].detach()
        metrics_g[METRICS_PRED_NDX, start_ndx:end_ndx] = propability_g[:, 1].detach()
